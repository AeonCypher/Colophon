...
from colophon.build import build_site, load_project_contexts, resolve_required_vendor_assets
...
from colophon.deploy.transports import is_safe_remote_purge_path, planned_upload_actions, upload_site_directory, upload_with_sftp
from colophon.errors import AssetError, ContentError, DeployError, ExpressionResolutionError, ProjectConfigError, TemplateBuildError
from colophon.expressions import YAML_CONTEXT_FUNCTIONS, YAML_FUNCTIONS, expression_registry, import_python_module, module_yaml_functions, resolve_env_references, resolve_site_expressions, resolve_yaml_expressions
...
class ColophonTests(unittest.TestCase):
    ...
    def test_context_yaml_functions_resolve_markdown_frontmatter(self) -> None:
        with isolated_paths():
            hooks = ROOT / "site_hooks.py"
            hooks.write_text(
                """
import datetime as dt
...
def series(context):
    return {
        ...
        "source": context.source_file.content_path,
        "immutable": context.data.__class__.__name__,
    }
...
""",
            ...
            project = replace(active_project(), python_modules=(hooks,))

            _, contexts = load_project_contexts(project)
            post = next(context for context in contexts if context.route.url_path == "/posts/context/")

        self.assertEqual(post.data["date"], dt.date(2026, 2, 3))
        self.assertEqual(post.data["summary"], "The first paragraph becomes the summary.")
        self.assertEqual(
            post.data["series"],
            {
                "route": "/posts/context/",
                "source": "posts/context.md",
                "immutable": "mappingproxy",
            },
        )

    def test_context_yaml_function_rejected_without_page_context(self) -> None:
        with patch.dict(YAML_CONTEXT_FUNCTIONS, {"needs_page": lambda context: "x"}, clear=False):
            with self.assertRaisesRegex(
                ExpressionResolutionError,
                r"site\.title.*requires page context",
            ):
                resolve_yaml_expressions(
                    {"site": {"title": "python::needs_page"}},
                    path="",
                )

    def test_deploy_config_rejects_context_yaml_function(self) -> None:
        raw = {
            "deploy": {
                "targets": {
                    "production": {
        ...
        with patch.dict(YAML_CONTEXT_FUNCTIONS, {"needs_page": lambda context: "x"}, clear=False):
            with self.assertRaisesRegex(
                ExpressionResolutionError,
                r"deploy\.targets\.production\.password.*requires page context",
            ):
                validate_deploy_config(raw)
    ...
    def test_deploy_config_validates_defaults_and_redacts_secrets(self) -> None:
        ...
        with patch.dict(
            os.environ,
            {
                ...
                "EXAMPLE_FTP_PASSWORD": "ftp-password",
            },
            clear=False,
        ):
...
""",
        ...
        self.assertEqual(timeline["options"]["profileName"], "@LibertAiTech")
...
site:
  ...
  signal_line:
    ...
    MOON: python::moon
    FEED: HAND-CURATED
    SPELL CACHE: python::temperature
""",
                encoding="utf-8",
            )
            (CONTENT / "index.yml").write_text(
...
""",
    ...
    def test_sftp_missing_extra_mentions_distribution_name(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            source = Path(tmp) / "site"
            source.mkdir()
            ...
            with patch.dict(sys.modules, {"paramiko": None}):
                with self.assertRaisesRegex(DeployError, r"colophon-site\[sftp\]"):
                    upload_with_sftp(target, source, dry_run=False)
