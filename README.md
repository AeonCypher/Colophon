...
# Why Use colophon?
| Area                                       | Colophon's angle                                        | How that differs from common SSGs            |
| ------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Site/generator separation**              | The generator is designed to live outside the site repo, with explicit `--config`, project paths, and immutable project loading.                                                       | Many SSG workflows assume the site project contains the generator config, theme, dependencies, and build context in one repo. Colophon seems designed for a split "engine + site" workspace. |
| **Structured YAML as first-class content** | YAML pages are not just metadata; they can drive homepages, feature lists, sidebars, collections, route rules, and site-wide data.                                                     | Jekyll/Hugo/Eleventy/Astro all support Markdown/front matter, but Colophon leans harder into YAML as structured site content, not merely post metadata.                                      |
| **Trusted Python hooks from YAML**         | YAML can call declared local Python functions like `python::demo_status`, and can pull environment variables with `env::...`.                                                          | This is more "site-local programmable data layer" than a normal plugin system. It is powerful, but intentionally trusted/local rather than sandboxed.                                      |
| **Automation-friendly build output**       | Builds can emit a manifest with generated pages, posts, routes, source chains, SHA-256 hashes, assets, warnings, and feed/archive/tag outputs.                                         | That is unusually explicit for a small SSG. It suggests Colophon cares about CI, deploy validation, reproducibility, and tooling around the build, not just HTML output.                    |
| **Strictness**                             | It validates config strictly, fails on missing images/assets, reports typed error categories, and avoids tracebacks by default.                                                        | Many SSGs are more permissive or theme/plugin-driven. Colophon seems to prefer "fail loudly before deploy."                                                                                  |
| **Built-in image registry**                | Images can be declared by logical name in `content/images.yml`, with variants and template helpers returning dimensions, alt text, URLs, fit, ratio, fallback, and existence metadata. | Hugo/Astro have strong asset pipelines, but for a small Python SSG this logical image registry is a distinctive built-in workflow.                                                          |
| **Vendor asset policy**                    | It has a built-in local/CDN/download abstraction for browser assets like Web Awesome, DOMPurify, Font Awesome, and Mastodon-related scripts.                                           | That is more opinionated than most minimal SSGs, and less general than Astro/Vite-style frontend bundling.                                                                                   |
| **Mastodon-native publishing support**     | It includes static-site-friendly Mastodon timeline/comments support and deploy steps that can render Mastodon post text and enable comments.                                           | That is quite niche and probably one of its most distinctive product opinions. Most SSGs leave comments/social posting to third-party integrations.                                        |
| **Old-school deploy included**             | It supports config-driven deploy via FTP, FTPS, SFTP, and SSHFS, with dry-run and purge safety checks.                                                                                 | Modern SSGs often assume GitHub Pages, Netlify, Vercel, Cloudflare Pages, or custom CI. Colophon seems friendlier to shared hosting / hand-managed servers.                            |
| **Atomic watch rebuilds**                  | In watch mode, it keeps serving the previous successful output if a rebuild fails.                                                                                                     | That is a nice developer-experience detail: failed rebuilds do not necessarily break the local served site. |                                                                            
